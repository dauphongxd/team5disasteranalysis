...
# 4. MySQL Database Conig
db_config = {
    'host': 'localhost',
    'user': os.getenv('DB_USER'),
    'password': os.getenv('DB_PW'),  
    'database': 'disaster_feed_db'  
}

# Function to insert post data into MySQL database
def insert_post_data(post_data):
    try:
        conn = mysql.connector.connect(**db_config)
        cursor = conn.cursor()

        insert_query = """
        INSERT INTO posts (uri, handle, display_name, text, demojized_text, timestamp, avatar, media, predicted_disaster_type, confidence_score, is_disaster)
        ...
        cursor.execute(insert_query, (
            ...
            post_data["timestamp"],
            post_data["avatar"],
            #json.dumps(post_data["media"]),  # Convert list to JSON string
            post_data["predicted_disaster_type"],
            post_data["confidence_score"],
            post_data["is_disaster"]
        ))
...
# 5. Real-time Processing Function
def process_feed():
    last_processed = None
